...
def torch_distributed_zero_first(local_rank: int):
    ...
    #  torch_distributed_zero_first是在create_dataloader函数中调用的，如果
    #  执行create_dataloader()函数的进程不是主进程，即rank不等于0或者-1，
    ...
    # 然后其处理结束之后会接着遇到torch.distributed.barrier()，此时，所有进程都到达了当前的栅栏处，
    # 这样所有进程就达到了同步，并同时得到释放。
