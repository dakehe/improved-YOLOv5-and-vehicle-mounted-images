...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True):
        # path：yolov5-master/data/images/crazing_30.jpg
        p = str(Path(path).resolve())  # os-agnostic absolute path
        # 转化为绝对路径 D:\长沙理工大学研究生\裂缝检测\钢材裂缝检测\yolov5\yolov5-master\data\images\crazing_30.jpg
        if '*' in p:
            files = sorted(glob.glob(p, recursive=True))  # glob 没有*号
        elif os.path.isdir(p):
            files = sorted(glob.glob(os.path.join(p, '*.*')))  # dir 也不是文件夹
        elif os.path.isfile(p):
            files = [p]  # files是文件 [D:\长沙理工大学研究生\裂缝检测\钢材裂缝检测\yolov5\yolov5-master\data\images\crazing_30.jpg]
        else:
            raise Exception(f'ERROR: {p} does not exist')

        images = [x for x in files if x.split('.')[-1].lower() in IMG_FORMATS] # 有img格式的就不变
        videos = [x for x in files if x.split('.')[-1].lower() in VID_FORMATS]
        ni, nv = len(images), len(videos) # 一张图片就是ni=1，nv=0
